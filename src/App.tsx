...
import { getLocations, getTypes } from "./network/api/axios.custom";
...
function App() {
  ...
  const apiLoad = async () => {
    const LresponseProm = getLocations();
    const TresponseProm = getTypes();

    const [Lresponse, Tresponse] = await Promise.all([LresponseProm, TresponseProm]);
    setLocations(Lresponse);
    setTypes(Tresponse);
  }
